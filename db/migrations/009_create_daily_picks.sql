...
-- rank 不存储，查询时用 ORDER BY score 或窗口函数实时计算。
-- 共振统计不存储，用 view 聚合（避免 fact table 混入 derived data）。
...
CREATE INDEX idx_daily_picks_date_strategy_score
    ON daily_picks (trade_date, strategy, score DESC);  -- 单策略 top N 查询
