...
- vol_ma：基于 daily_cn.vol 的 5 日 / 10 日移动平均，单位=股，成交量无复权语义，不足窗口期严格为 NULL
- vol_ratio：基于 daily_cn.vol / vol_ma5，单位=无，vol_ma5 为 0 或窗口不足时为 NULL
- turnover_rate_ma：基于 daily_basic_cn.turnover_rate 的 5 日移动平均，单位=%（3.5 表示 3.5%），不足窗口期严格为 NULL
- turnover_rate_ratio：基于 turnover_rate / turnover_rate_ma5，单位=无，分母为 0 或窗口不足时为 NULL
- pe_pb_quantile：基于 daily_basic_cn.pe_ttm / pb 的 60 日滚动分位，单位=小数 [0,1]，窗口内有效样本不足 60 时严格为 NULL

调用方需提供 `ts_code` / `trade_date`，以及对应函数所需的 `vol`、`turnover_rate`、`pe_ttm`、`pb` 列。
"""
...
def compute_pe_pb_quantile(df: pd.DataFrame) -> pd.DataFrame:
    ...
    result["pe_ttm_pct_60"] = result.groupby("ts_code", sort=False)["pe_ttm"].transform(_rolling_pct_60)
    result["pb_pct_60"] = result.groupby("ts_code", sort=False)["pb"].transform(_rolling_pct_60)
...
def _rolling_pct_60(s: pd.Series) -> pd.Series:
    return s.rolling(60, min_periods=60).apply(lambda x: (x.rank(method="average").iloc[-1] - 1) / 59)
