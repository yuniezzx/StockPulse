...
- main_net：基于 moneyflow_cn 大单 + 特大单计算主力净流入金额、占成交额比例与 5 日均值，金额单位=元，比例单位=无，不足窗口期严格为 NULL
- retail_net：基于 moneyflow_cn 小单 + 中单计算散户净流入金额，金额单位=元，输入缺失时自然传播 NULL

调用方需提供 `ts_code` / `trade_date`，以及对应函数所需的 moneyflow_cn 金额列；main_net 额外需要 daily_cn.amount。
"""
...
def compute_main_net(df: pd.DataFrame) -> pd.DataFrame:
    ...
    result["main_net_amount_ma5"] = result.groupby("ts_code", sort=False)["main_net_amount"].transform(
